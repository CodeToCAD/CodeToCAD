...
def directory_to_snake_case(
    directory_path: str, replace_definitions_only: bool = False, overwrite: bool = True
):
...
def file_to_snake_case(
    filepath: str, replace_definitions_only: bool = False, overwrite: bool = True
):
    """
    ...
    file_data = FileData(file_content=file_content, definitions=definitions)
    ...
    if overwrite:
        ...
        filepath_snake_case = filepath.replace(filename, to_snake_case(filename))
...
def parse_definition(
    definition: ast.stmt, file_data: FileData, replace_definitions_only: bool = False
):
...
def method_to_snake_case(
    method, file_data: FileData, replace_definitions_only: bool = False
):
...
def main():
    ...
    parser.add_argument(
        "-o", "--overwrite", action="store_true", help="Overwrite source file."
    )
    parser.add_argument(
        "-f", "--file", type=str, required=False, help="Filepath to convert"
    )
    parser.add_argument(
        "-d", "--directory", type=str, required=False, help="Directory to convert."
    )
    ...
    if directorypath:
        return directory_to_snake_case(
            directorypath, replace_definitions_only, overwrite
        )
