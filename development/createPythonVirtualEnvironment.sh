...
# e.g. run it with sh createPythonVirtualEnvironment /path/to/python
if [ ! -z $1 ]; then
    alias python=$1
...
if [[ $PYTHON_VERSION == "bad version" ]]; then
    echo "Please run this with at least Python 3.10. If you have multiple versions of Python installed, you may run this script using 'sh createPythonVirtualEnvironment /path/to/python'"
    exit 1
fi
...
# While we're at it, VSCode's .env file on Windows uses different syntax to linux, so we're going to link the correct file.
if [ -d "$VENV_DIR/Scripts" ]; then
    ln -sf "$SCRIPT_DIR/.pythonpath.windows.env" "$SCRIPT_DIR/.pythonpath.env" 
else
    ln -sf "$SCRIPT_DIR/.pythonpath.linux.env" "$SCRIPT_DIR/.pythonpath.env" 
fi
