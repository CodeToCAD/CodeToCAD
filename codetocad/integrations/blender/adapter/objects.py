...
from codetocad.integrations.blender.adapter.addons import enable_addon
from codetocad.integrations.blender.blender_definitions import (
...
def blender_primitive_function(
    primitive: BlenderObjectPrimitiveTypes, dimensions: list[float], **kwargs
):
    primitive_name = primitive.default_name_in_blender()

    # Make sure an object or mesh with the same name don't already exist.
    blender_object = bpy.data.objects.get(primitive_name)
    blender_mesh = bpy.data.meshes.get(primitive_name)

    assert (
        blender_object is None
    ), f"An object with name {primitive_name} already exists."

    orphanMeshMessage = ""
    if blender_mesh is not None and blender_mesh.users == 0:
    ...
    assert (
        blender_mesh is None
    ), f"A mesh with name {primitive_name} already exists. {orphanMeshMessage}"

    if primitive == BlenderObjectPrimitiveTypes.cube:
        return bpy.ops.mesh.primitive_cube_add(size=1, scale=dimensions[:3], **kwargs)

    if primitive == BlenderObjectPrimitiveTypes.cone:
        return bpy.ops.mesh.primitive_cone_add(
            radius1=dimensions[0],
            radius2=dimensions[1],
            depth=dimensions[2],
            **kwargs,
        )
    ...
    if primitive == BlenderObjectPrimitiveTypes.cylinder:
        return bpy.ops.mesh.primitive_cylinder_add(
            radius=dimensions[0], depth=dimensions[1], **kwargs
        )

    if primitive == BlenderObjectPrimitiveTypes.torus:
        return bpy.ops.mesh.primitive_torus_add(
            mode="EXT_INT",
            abso_minor_rad=dimensions[0],
            abso_major_rad=dimensions[1],
            **kwargs,
        )

    if primitive == BlenderObjectPrimitiveTypes.sphere:
        return bpy.ops.mesh.primitive_ico_sphere_add(radius=dimensions[0], **kwargs)

    if primitive == BlenderObjectPrimitiveTypes.uvsphere:
        return bpy.ops.mesh.primitive_uv_sphere_add(radius=dimensions[0], **kwargs)

    if primitive == BlenderObjectPrimitiveTypes.circle:
        return bpy.ops.mesh.primitive_circle_add(radius=dimensions[0], **kwargs)

    if primitive == BlenderObjectPrimitiveTypes.grid:
        return bpy.ops.mesh.primitive_grid_add(size=dimensions[0], **kwargs)

    if primitive == BlenderObjectPrimitiveTypes.monkey:
        return bpy.ops.mesh.primitive_monkey_add(size=dimensions[0], **kwargs)

    if primitive == BlenderObjectPrimitiveTypes.empty:
        return bpy.ops.object.empty_add(radius=dimensions[0], **kwargs)
...
def add_primitive(
    ...
    **kwargs,
    ...
    # Add the object:
    blender_primitive_function(primitive_type, dimensions, **kwargs)
...
def create_gear(
    ...
    skew_angle: float = 0,
    conical_angle: float = 0,
    crown_angle: float = 0,
):
    enable_addon("add_mesh_extra_objects")

    if addendum > outer_radius / 2:
        addendum = outer_radius / 2
    if inner_radius > outer_radius:
        inner_radius = outer_radius
    if dedendum + inner_radius > outer_radius:
        dedendum = outer_radius - inner_radius
