...
from codetocad.integrations.blender.adapter.addons import (
    enable_addon,
)
from codetocad.integrations.blender.adapter.collections import (
    assign_object_to_collection,
)
from codetocad.integrations.blender.adapter.context import update_view_layer
from codetocad.integrations.blender.adapter.objects import (
    create_object,
)
from codetocad.integrations.blender.adapter.objects_context import (
...
def set_curve_extrude_property(curve_name: str, length: float):
    """
    Changes a curve's Geometry -> Extrude property.
    """
    curve = get_curve(curve_name)
    curve.extrude = length


def set_curve_offset_geometry(curve_name: str, offset: float):
    """
...
def create_curve(
    ...
    points: Sequence[list[float] | Point],
    interpolation=64,
    is_3d=False,
    order_u: int = 2,
) -> """tuple[
    ...
    curve_data.floats = "3D" if is_3d else "2D"
    ...
    points_expanded: List = [
        [float for float in point.to_list()] for point in points_parsed
    ]
...
class BlenderCurvePrimitives:
    ...
    def create_line_to(end_location, **kwargs):
        create_simple_curve(
            ...
            Simple_endlocation=end_location,
            use_cyclic_u=False,
            **kwargs,
        )
    ...
    def create_angle(length, angle, **kwargs):
        create_simple_curve(
            ...
            Simple_length=length,
            Simple_angle=math.degrees(angle),
            use_cyclic_u=False,
            **kwargs,
        )
    ...
    def create_spiral(
        ...
        **kwargs,
        ...
        enable_addon("add_curve_extra_objects")

        heightMeters = height

        radiusMeters = radius

        radius_endMeters = radius_end if radius_end else None

        radiusDiff = (
            0 if radius_endMeters is None else (radius_endMeters - radiusMeters)
        )
